...
class ImagePreprocessor:
    ...
    def _analyze_quality(self, image: Image.Image) -> ImageQualityReport:
        ...
        # Extrair GPS estruturado
        gps_lat = exif_data.get('GPS_Lat')
        gps_lon = exif_data.get('GPS_Lon')
        gps_map_url = exif_data.get('Google_Maps', '')
        gps_osm_url = exif_data.get('OpenStreetMap', '')
    ...
    def _calculate_contrast(self, image: Image.Image) -> float:
        ...
        try:
            if NUMPY_AVAILABLE:
                gray = image.convert('L')
                img_array = np.array(gray)
                return float(min(1.0, np.std(img_array) / 128.0))  # Normalizado e clampado
            else:
                # Fallback
                grayscale = image.convert('L')
                histogram = grayscale.histogram()
    ...
    def _extract_exif(self, image: Image.Image) -> dict:
        ...
        try:
            ...
            interesting_tags = [
                ...
                'Software', 'LensModel', 'LensSpecification',
                'WhiteBalance', 'Flash', 'ExposureMode', 'ExposureProgram',
                'SceneCaptureType', 'Orientation', 'ResolutionUnit',
                'XResolution', 'YResolution', 'ColorSpace',
                'DigitalZoomRatio', 'GPSAltitude', 'GPSAltitudeRef',
                'GPSTimeStamp', 'GPSDateStamp', 'UserComment',
                'HostComputer', 'Artist', 'Copyright',
            ]
                ...
                try:
                    ...
                    if "GPS" in exif_dict and exif_dict["GPS"]:
                        ...
                        if lat is not None and lon is not None:
                            ...
                            exif_data['OpenStreetMap'] = f"https://www.openstreetmap.org/?mlat={lat:.6f}&mlon={lon:.6f}#map=17/{lat:.6f}/{lon:.6f}"
                except Exception as e:
...
def quick_analyze(
    ...
    Args:
        image: Imagem PIL.
        detect_faces: Se True, executa detecção de faces (requer OpenCV).
        analyze_colors: Se True, extrai cores dominantes (requer numpy).
    """
    ...
    if result.quality_report:
        return {
            ...
            "has_faces": result.quality_report.has_faces,
            "face_count": result.quality_report.face_count,
            "dominant_colors": result.quality_report.dominant_colors,
            "recommendations": result.quality_report.recommendations,
            "exif_data": result.quality_report.exif_data,
            "gps_lat": result.quality_report.gps_lat,
            "gps_lon": result.quality_report.gps_lon,
            "gps_map_url": result.quality_report.gps_map_url,
            "gps_osm_url": result.quality_report.gps_osm_url,
        }
