...
try:
    ...
    EXIF_ANALYZER_AVAILABLE = True
except ImportError:
    EXIF_ANALYZER_AVAILABLE = False
    EXIFAnalyzer = None

try:
    from clone_detector import CloneDetector

    CLONE_DETECTOR_AVAILABLE = True
except ImportError:
    CLONE_DETECTOR_AVAILABLE = False
    CloneDetector = None
...
try:
    ...
    SHADOW_ANALYZER_AVAILABLE = True
except ImportError:
    SHADOW_ANALYZER_AVAILABLE = False
    ShadowAnalyzer = None
...
class OpenAIClient:
    ...
    def stream_image(self, image_data: ImageData, system_prompt: str, model: str = OPENAI_MODEL):
        """Analisa com OpenAI emitindo os tokens conforme chegam."""
        if not self.is_available():
            yield "[Cliente OpenAI não disponível]"
            return
...
class OllamaClient:
    ...
    def stream_image(self, image_data: ImageData, system_prompt: str, model: str):
        """Analisa com um modelo Ollama emitindo os tokens conforme chegam.

        Blocos <think> são suprimidos durante o stream e o texto final passa pelo mesmo
        _strip_think_blocks usado no caminho não-streaming.
        """
        if not self.is_available():
        ...
        try:
            ...
            for chunk in stream:
                ...
                # Enquanto um <think> estiver aberto, segura a saída.
                text_so_far = "".join(buffer)
                if "<think>" in text_so_far and "</think>" not in text_so_far:
                    continue

                visible = _strip_think_blocks(text_so_far)
...
class AnalysisPipeline:
    ...
    def __init__(self, analysis_mode: str = "geral", ocr_engine: str = "glm-ocr", yolo_model: str = "yolo11s"):
        ...
        self._loaded_ollama_models: List[str] = []
    ...
    def _build_quality_and_exif(self, image: Image.Image) -> Tuple[str, str]:
        ...
        face_count = quality_info.get("face_count", 0)
        if face_count:
            lines.append(f"Faces detectadas: {face_count}")

        dominant_colors = quality_info.get("dominant_colors", [])
        if dominant_colors:
            color_hexes = [c.get("hex", "") for c in dominant_colors[:5] if c.get("hex")]
            if color_hexes:
                lines.append(f"Cores dominantes: {', '.join(color_hexes)}")

        blur_score = quality_info.get("blur_score", 0)
        contrast = quality_info.get("contrast", 0)
        sharpness = quality_info.get("sharpness", 0)
        composite = self._compute_quality_score(blur_score, brightness, contrast, sharpness)
        lines.append(f"Score de qualidade (0-100): {composite:.0f}")

        gps_lat = quality_info.get("gps_lat")
        gps_lon = quality_info.get("gps_lon")
        if gps_lat is not None and gps_lon is not None:
            lines.append(f"GPS: {gps_lat:.6f}, {gps_lon:.6f}")
            osm_url = quality_info.get("gps_osm_url", "")
            if osm_url:
                lines.append(f"Mapa: {osm_url}")
    ...
    @staticmethod
    def _compute_quality_score(blur_score: float, brightness: float, contrast: float, sharpness: float) -> float:
        blur_norm = min(1.0, blur_score / 300.0)
        bright_penalty = 1.0 - abs(brightness - 0.5) * 2.0
        bright_penalty = max(0.0, bright_penalty)
        score = (
            blur_norm * 40
            + min(1.0, sharpness * 10) * 25
            + bright_penalty * 15
            + min(1.0, contrast) * 20
        )
    ...
    def _build_ela_context(self, image: Image.Image) -> str:
        if not ELA_AVAILABLE or ELAAnalyzer is None:
            return ""
    ...
    @staticmethod
    def _provenance_label(image: Image.Image, is_original: bool) -> str:
        """Descreve sobre qual versão da imagem uma análise forense foi executada."""
        width, height = image.size
        if is_original:
            return f"(origem: arquivo original, {width}x{height})"
        return f"(origem: JPEG reduzido q{JPEG_QUALITY}, {width}x{height})"

    def _build_forensic_context(self, image: Image.Image, is_original: bool = True) -> str:
        """Executa todas as análises forenses e retorna contexto combinado.

        `image` deve ser a imagem ORIGINAL carregada do disco. Rodar estas análises
        sobre o JPEG recomprimido de `prepare_for_api` invalida os resultados: o EXIF
        é descartado no reencode e a esteganografia LSB não sobrevive à recompressão.
        """
        sections = []
        origem = self._provenance_label(image, is_original)

        # ELA
        ela_summary = self._build_ela_context(image)
        if ela_summary:
            sections.append(f"### Error Level Analysis (ELA) {origem}\n{ela_summary}")

        # JPEG Ghost
        if ELA_AVAILABLE and ELAAnalyzer is not None:
            try:
                ela = ELAAnalyzer(quality=95, scale=15)
                ghost = ela.ghost_analysis(image)
                ghost_summary = ghost.get_summary()
                if ghost_summary:
                    sections.append(f"### JPEG Ghost Analysis {origem}\n{ghost_summary}")
            except Exception as exc:
                logger.debug("Falha no ghost analysis: %s", exc)

        # EXIF Tampering
        if EXIF_ANALYZER_AVAILABLE and EXIFAnalyzer is not None:
            try:
                exif_analyzer = EXIFAnalyzer()
                exif_result = exif_analyzer.analyze(image)
                exif_summary = exif_result.get_summary()
                if exif_summary:
                    sections.append(f"### Análise de Metadados EXIF {origem}\n{exif_summary}")
            except Exception as exc:
                logger.debug("Falha no EXIF analysis: %s", exc)

        # Clone Detection
        if CLONE_DETECTOR_AVAILABLE and CloneDetector is not None:
            try:
                clone_detector = CloneDetector()
                clone_result = clone_detector.analyze(image)
                clone_summary = clone_result.get_summary()
                if clone_summary:
                    sections.append(f"### Copy-Move Detection {origem}\n{clone_summary}")
            except Exception as exc:
                logger.debug("Falha no clone detection: %s", exc)

        # Steganography
        if STEGO_DETECTOR_AVAILABLE and StegoDetector is not None:
            try:
                stego_detector = StegoDetector()
                stego_result = stego_detector.analyze(image)
                stego_summary = stego_result.get_summary()
                if stego_summary:
                    if not is_original:
                        stego_summary += (
                            "\n⚠️ Resultado não conclusivo: a análise LSB rodou sobre uma versão "
                            "recomprimida, e a recompressão JPEG destrói o bit menos significativo."
                        )
                    sections.append(f"### Steganografia LSB {origem}\n{stego_summary}")
            except Exception as exc:
                logger.debug("Falha no stego detection: %s", exc)

        # Shadow Consistency
        if SHADOW_ANALYZER_AVAILABLE and ShadowAnalyzer is not None:
            try:
                shadow_analyzer = ShadowAnalyzer()
                shadow_result = shadow_analyzer.analyze(image)
                shadow_summary = shadow_result.get_summary()
                if shadow_summary:
                    sections.append(f"### Consistência de Sombras {origem}\n{shadow_summary}")
            except Exception as exc:
                logger.debug("Falha no shadow analysis: %s", exc)

        if not sections:
            return ""

        return "# DADOS FORENSES AUTOMATIZADOS\n\n" + "\n\n".join(sections)
    ...
    def prepare_report(
        ...
        # Versão reduzida/recomprimida: serve para o LLM e para o YOLO, que não dependem
        # de metadado. As análises de EXIF e forenses precisam do ORIGINAL (loaded_image).
        image_for_inference = Image.open(BytesIO(image_data.jpeg_bytes))
        ...
        # Qualidade/EXIF e forenses são gravadas juntas no cache (quality_result já traz o
        # bloco forense concatenado), então precisam da MESMA condição de reaproveitamento.
        # Condições divergentes faziam as forenses serem puladas em silêncio.
        context_is_cached = bool(
            cached_intermediate
            and cached_intermediate.get("quality_result")
            and cached_intermediate.get("exif_data")
        )

        quality_exif_start = time.perf_counter()
        if context_is_cached:
            ...
            quality_str, exif_str = self._build_quality_and_exif(loaded_image)
        ...
        if not self._should_run_yolo():
            ...
            yolo_str = self._build_yolo_summary(image_for_inference)
            self._record_stage(telemetry, "yolo", yolo_start, detail=self.yolo_model)

        ela_start = time.perf_counter()
        if context_is_cached:
            forensic_str = ""
            forensic_status = "cached"
        else:
            forensic_str = self._build_forensic_context(loaded_image, is_original=True)
            forensic_status = "completed" if forensic_str else "skipped"
        self._record_stage(telemetry, "forensic", ela_start, status=forensic_status, detail=self.analysis_mode)
        if forensic_str:
    ...
    def stream_model(self, report: ImageAnalysisReport, model_id: str, model_type: str):
        """Emite a análise token a token para um único modelo.

        Usado na análise de imagem única da UI, onde o feedback incremental importa.
        Em lote não compensa, e o caminho não-streaming continua sendo o padrão.
        """
        if model_type == "openai":
            yield from self.openai_client.stream_image(report.image, report.prompt_used, model_id)
        else:
            yield from self.ollama_client.stream_image(report.image, report.prompt_used, model_id)

    def finalize_streamed_analysis(
        self,
        ...
        """Monta o AnalysisResult de um stream já consumido e roda o pós-processamento."""
        cleaned = _strip_think_blocks(content or "")
        analysis = AnalysisResult(
            model_name=model_id,
            model_type=model_type,
            success=bool(cleaned) and not cleaned.startswith("[Erro"),
            content=cleaned,
            error="" if cleaned else "Modelo retornou resposta vazia",
            processing_time=elapsed,
        )
        self._run_post_processing(report, analysis)
        self._register_loaded_models([(model_id, model_type)])
        return analysis
    ...
    def _cached_artifacts_available(cached_report: Path, export_formats: List[str]) -> bool:
        # Path.with_suffix opera sobre o ÚLTIMO ponto do nome, então não serve aqui:
        # "IMG_2024.01.15_gpt54-mini.md" viraria "IMG_2024.01.pdf". Nomes com ponto são
        # a regra em exports de WhatsApp/câmera, e o cache nunca dava hit para eles.
        stem = cached_report.name[: -len(cached_report.suffix)] if cached_report.suffix else cached_report.name
        for fmt in export_formats:
            if not cached_report.with_name(f"{stem}.{fmt}").exists():
                return False
        return True
    ...
    def process_image(
        ...
        # O unload da VRAM NÃO acontece aqui: descarregar por imagem forçava o Ollama a
        # recarregar o modelo do disco a cada arquivo do lote. Quem orquestra o lote chama
        # unload_models() uma única vez ao final.
        self._register_loaded_models(pending_models)

        return result

    def _register_loaded_models(self, models: List[Tuple[str, str]]) -> None:
        """Anota quais modelos Ollama ficaram residentes na VRAM durante a execução."""
        for model_id, model_type in models:
            if model_type == "ollama" and model_id not in self._loaded_ollama_models:
                self._loaded_ollama_models.append(model_id)

        if self.ocr_engine != "none":
            ocr_model = OCR_MODEL if self.ocr_engine == "glm-ocr" else OCR_MODEL_ALT
            if ocr_model not in self._loaded_ollama_models:
                self._loaded_ollama_models.append(ocr_model)

    def unload_models(self) -> List[str]:
        """Libera da VRAM todos os modelos Ollama usados por este pipeline.

        Deve ser chamado uma vez ao término do lote (ou da análise avulsa), nunca por imagem.
        Retorna a lista de modelos descarregados.
        """
        if not self._loaded_ollama_models:
            return []

        unloaded = list(self._loaded_ollama_models)
        self.ollama_client.unload_models(unloaded)
        self._loaded_ollama_models.clear()
        return unloaded