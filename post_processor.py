...
# RG (formato genérico BR): 00.000.000-0 ou 000000000
RG_PATTERN = re.compile(
    r'\b(\d{1,2}[./]?\d{3}[./]?\d{3}[-]?\d{1})\b'
)

# Chave PIX aleatória (32 caracteres hex/alnum)
PIX_RANDOM_PATTERN = re.compile(
    r'\b([0-9a-fA-F]{32})\b'
)

# Linha digitável de boleto (44 ou 47 dígitos)
BOLETO_PATTERN = re.compile(
    r'\b(\d{12}\.\d{1}\s\d{11}\.\d{1}\s\d{11}\.\d{1}\s\d{11}\.\d{1}\s\d{1})\b'
    r'|'
    r'\b(\d{47})\b'
)

# Endereço Bitcoin (Legacy, P2SH, Bech32)
BITCOIN_PATTERN = re.compile(
    r'\b((?:bc1|[13])[a-zA-HJ-NP-Z0-9]{25,62})\b'
)

# Conta bancária BR (agência + conta, formatos comuns)
ACCOUNT_PATTERN = re.compile(
    r'(?:ag(?:ência|encia)?[.:\s]*(\d{3,5}[-]?\d?))'
    r'[^\d]+(?:conta[.:\s]*(\d{3,8}[-]?\d))',
    re.IGNORECASE
)

# Timestamps de câmera de vigilância (DVR)
DVR_TIMESTAMP_PATTERNS = [
    # 2024-01-15 14:30:00 ou 2024/01/15 14:30:00
    re.compile(r'\b(\d{4}[-/]\d{2}[-/]\d{2}\s+\d{2}:\d{2}:\d{2})\b'),
    # 15-01-2024 14:30:00
    re.compile(r'\b(\d{2}[-/]\d{2}[-/]\d{4}\s+\d{2}:\d{2}:\d{2})\b'),
    # 2024-01-15 14:30 (sem segundos)
...
def validate_boleto(line: str) -> bool:
    """Validação básica de linha digitável de boleto (apenas contagem de dígitos)."""
    digits = re.sub(r'\D', '', line)
    return len(digits) in (44, 47, 48)
...
class PostProcessor:
    ...
    DOC_TYPE_KEYWORDS = {
        ...
        "cnpj": ["cnpj", "empresa", "razão social", "inscrição estadual"],
        "cnh": ["carteira de habilitação", "cnh", "habilitação"],
        "comprovante": ["comprovante", "recibo", "nota fiscal", "nf-e", "boleto", "linha digitável"],
        ...
        "vigilancia": ["câmera", "camera", "cctv", "vigilância", "monitoramento", "dvr"],
        "criptomoeda": ["bitcoin", "btc", "carteira digital", "exchange", "cripto"],
    }

    # Classificação de ameaças/golpes
    THREAT_KEYWORDS = {
        "phishing": ["phishing", "falso", "clique aqui", "senha", "atualizar cadastro", "urgente"],
        "fraude_financeira": ["pix", "transferência", "boleto falso", "código de barras", "qr code", "chave pix", "conta bancária", "agência", "saque", "depósito"],
        "ameaca": ["ameaça", "matar", "morrer", "vou te", "cuidado", "aviso"],
        "extorsao": ["extorsão", "pagar", "resgate", "vazamento", "nude", "íntim"],
        "falsificacao": ["falsificação", "adulterado", "manipulado", "editado", "photoshop", "montagem"],
        ...
        "lavagem": ["lavagem", "cripto", "bitcoin", "mixer", "tumbler", "exchange"],
    }
    ...
    def process(
        ...
        # 5. Extrair timeline (inclui timestamps de DVR/câmera de vigilância)
        all_text = f"{ocr_text}\n{llm_analysis}\n{exif_data}"
        result.timeline = self.extract_timeline(all_text)
        dvr_events = self.extract_dvr_timestamps(ocr_text)
        if dvr_events:
            result.timeline.extend(dvr_events)
            result.timeline.sort(key=lambda e: e.parsed_dt or datetime.max)
    ...
    def extract_entities(self, text: str) -> List[ExtractedEntity]:
        ...
        # RGs (apenas se parecer RG, não CPF — filtrar por tamanho)
        ...
        # Chaves PIX aleatórias (32 chars hex)
        for match in PIX_RANDOM_PATTERN.finditer(text):
            value = match.group(1)
            entities.append(ExtractedEntity(
                type="pix_key",
                value=value,
        ...
        for match in ACCOUNT_PATTERN.finditer(text):
            ag = match.group(1) or ""
            cc = match.group(2) or ""
            value = f"Ag: {ag} Conta: {cc}"
            entities.append(ExtractedEntity(
                type="bank_account",
                value=value,
                normalized=f"{re.sub(r'\\D', '', ag)}-{re.sub(r'\\D', '', cc)}",
                valid=bool(ag and cc),
                source="mixed",
                context=text[max(0, match.start()-30):match.end()+30]
            ))
    ...
    def extract_dvr_timestamps(self, text: str) -> List[TimelineEvent]:
        """Extrai timestamps de câmera de vigilância (DVR) do texto OCR."""
        events = []

        for pattern in DVR_TIMESTAMP_PATTERNS:
            for match in pattern.finditer(text):
        ...
        if result.entities:
            ...
            for ent in result.entities:
                icon = {
                    ...
                    "phone": "📞", "email": "📧",
                    "ipv4": "🌐", "ipv6": "🌐", "url": "🔗",
                    "plate": "🚗", "pix_key": "🔑", "boleto": "🧾",
                    "bitcoin": "₿", "bank_account": "🏦",
                }.get(ent.type, "📎")
