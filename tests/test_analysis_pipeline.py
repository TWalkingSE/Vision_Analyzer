...
from runtime_config import OCR_MODEL
...
def test_process_image_nao_descarrega_vram_por_imagem(monkeypatch, tmp_path):
    """Regressão: o unload acontecia ao fim de CADA imagem.

    Num lote de 500 arquivos, o modelo de visão e o de OCR eram despejados e recarregados
    do disco 500 vezes. O unload passou a ser explícito, uma vez, ao término do lote.
    """
    from analysis_pipeline import AnalysisPipeline

    pipeline = AnalysisPipeline(analysis_mode="geral", ocr_engine="glm-ocr")

    descarregados = []
    monkeypatch.setattr(
        pipeline.ollama_client, "unload_models", lambda models: descarregados.append(list(models))
    )
    monkeypatch.setattr(pipeline, "preflight", lambda models, out: ([], []))
    monkeypatch.setattr(pipeline, "prepare_report", lambda path, cache_manager=None: (None, "stub"))

    for i in range(3):
        image = tmp_path / f"img{i}.jpg"
        image.write_bytes(b"x")
        pipeline.process_image(
            image_path=image,
            selected_models=[("qwen3-vl:8b", "ollama")],
            output_dir=tmp_path,
        )

    assert descarregados == [], "nenhum unload deve ocorrer durante o lote"

    # Uma imagem sem sucesso não deixa modelo residente; simula o registro e libera.
    pipeline._register_loaded_models([("qwen3-vl:8b", "ollama")])
    liberados = pipeline.unload_models()

    assert len(descarregados) == 1, "o unload deve acontecer uma única vez"
    assert "qwen3-vl:8b" in liberados
    assert OCR_MODEL in liberados, "o modelo de OCR também precisa sair da VRAM"
    assert pipeline.unload_models() == [], "unload repetido não deve refazer o trabalho"
