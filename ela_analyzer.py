...
@dataclass
class GhostRegion:
    """Região suspeita detectada por JPEG Ghost Analysis."""
...
@dataclass
class GhostResult:
    """Resultado de JPEG Ghost Analysis."""
    regions: list = field(default_factory=list)  # List[GhostRegion]
    verdict: str = "sem_indicios"
    quality_levels: list = field(default_factory=list)
    detail: str = ""

    def get_summary(self) -> str:
        if not self.regions:
            return f"JPEG Ghost: {self.verdict} — nenhuma região fantasma detectada."
        lines = [
            f"JPEG Ghost: {self.verdict}",
            f"Regiões suspeitas: {len(self.regions)}",
            f"Níveis de qualidade testados: {self.quality_levels}",
        ]
        for i, r in enumerate(self.regions[:5]):
            lines.append(
                f"  Região {i+1}: Q{r.quality_level} "
                f"em {r.bbox} (diff: {r.error_diff:.2f})"
            )
        if self.detail:
...
class ELAAnalyzer:
    ...
    def ghost_analysis(
        ...
        """
        JPEG Ghost Analysis: recomprime em múltiplos níveis de qualidade
        e identifica regiões onde o erro é desproporcionalmente baixo
        em um nível específico, indicando inserção de região com qualidade diferente.

        Args:
            image: Imagem PIL.
            quality_levels: Lista de qualidades JPEG para testar.
            block_size: Tamanho do bloco para análise regional.
            threshold: Diferença mínima de erro para considerar suspeito.

        Returns:
            GhostResult com regiões suspeitas.
        """
        if quality_levels is None:
            quality_levels = [90, 85, 80, 75, 70, 60]

        result = GhostResult(quality_levels=quality_levels)

        if not NUMPY_AVAILABLE:
            result.verdict = "inconclusivo"
            result.detail = "numpy indisponível"
            return result
        ...
        try:
            ...
            orig_array = np.array(image, dtype=np.float64)
            h, w = orig_array.shape[:2]

            # Recomprimir em cada nível e calcular erro
            errors = {}
            for q in quality_levels:
                ...
                diff = np.mean(np.abs(orig_array - recomp), axis=2)
                errors[q] = diff

            # Para cada bloco, verificar se o erro é anormalmente baixo
            # em um nível específico comparado aos outros
            for y in range(0, h - block_size + 1, block_size):
                for x in range(0, w - block_size + 1, block_size):
                    block_errors = {}
                    for q in quality_levels:
                        block_val = np.mean(errors[q][y:y+block_size, x:x+block_size])
                        block_errors[q] = block_val

                    # Calcular média e desvio dos erros do bloco
                    vals = list(block_errors.values())
                    ...
                    # Encontrar nível com erro mais anômalo (mais baixo que os outros)
                    for q, err in block_errors.items():
                        diff_from_mean = mean_err - err
                        if diff_from_mean > threshold and err < mean_err * 0.5:
                            # Esta região tem erro muito menor neste nível
                            # → pode ter sido inserida nesta qualidade
                            result.regions.append(GhostRegion(
                                bbox=(int(x), int(y), int(x + block_size), int(y + block_size)),
                                quality_level=q,
                                error_diff=float(diff_from_mean),
                            ))
                            break  # Um nível por bloco

            # Determinar veredicto
            if len(result.regions) >= 5:
                result.verdict = "suspeito"
                result.detail = (
                    f"{len(result.regions)} regiões com erro anômalo detectadas. "
                    f"Possível inserção de elementos em qualidade JPEG diferente."
                )
            elif len(result.regions) >= 1:
                result.verdict = "inconclusivo"
                result.detail = (
                    f"{len(result.regions)} região(ões) com anomalia de qualidade. "
                    f"Inconclusivo."
                )
            else:
                result.verdict = "sem_indicios"
                result.detail = "Nenhuma região fantasma detectada."

        except Exception as e:
            logger.warning(f"Erro no ghost analysis: {e}")
            result.verdict = "inconclusivo"
            result.detail = str(e)
