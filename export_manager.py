...
class ExportManager:
    ...
    @staticmethod
    def _chain_of_custody_telemetry(data: ReportData) -> str:
        """Gera tabela Markdown de telemetria da pipeline para cadeia de evidências."""
        if not data.pipeline_telemetry:
            return ""
        lines = [
            "### Telemetria da Pipeline",
            "",
            "| Etapa | Status | Duração | Detalhe |",
            "|:------|:-------|--------:|:--------|",
        ]
        for entry in data.pipeline_telemetry:
            ...
            detail = entry.get("detail", "") or "n/a"
            lines.append(f"| {stage} | {status} | {duration} ms | {detail} |")
        return "\n".join(lines)
...
"""
    ...
    def _json_payload(self, data: ReportData) -> Dict[str, Any]:
        ...
        # Extrair dados forenses do quality_result se presentes
        quality_text = data.quality_result or ""
        forensic_data = self._extract_forensic_from_quality(quality_text)
        ...
        return {
            ...
            "chain_of_custody": {
                ...
                "pipeline_telemetry": data.pipeline_telemetry,
                "preflight_warnings": data.preflight_warnings,
            },
            "image_integrity": forensic_data,
            "analysis": {
                "model": data.model,
    ...
    @staticmethod
    def _extract_forensic_from_quality(quality_text: str) -> Dict[str, Any]:
        """Extrai dados forenses estruturados do texto de quality_result."""
        if not quality_text:
            return {}

        result = {}
        # Detectar seção forense
        if "DADOS FORENSES AUTOMATIZADOS" in quality_text:
            result["forensic_analysis_present"] = True
            # Extrair veredictos individuais
            if "Veredicto ELA:" in quality_text:
                for line in quality_text.split("\n"):
                    line = line.strip()
                    if line.startswith("Veredicto ELA:"):
                        result["ela_verdict"] = line.replace("Veredicto ELA:", "").strip()
                    elif line.startswith("JPEG Ghost:"):
                        result["ghost_verdict"] = line.replace("JPEG Ghost:", "").strip()
                    elif line.startswith("EXIF") and "veredicto" in line.lower():
                        result["exif_verdict"] = line.split(":", 1)[-1].strip() if ":" in line else ""
                    elif line.startswith("Copy-Move:"):
                        result["clone_verdict"] = line.replace("Copy-Move:", "").strip()
                    elif line.startswith("Esteganografia LSB:"):
                        result["stego_verdict"] = line.replace("Esteganografia LSB:", "").strip()
                    elif line.startswith("Análise de Sombras:"):
                        result["shadow_verdict"] = line.replace("Análise de Sombras:", "").strip()
        else:
            result["forensic_analysis_present"] = False
...
| **Hash MD5** | `{data.hash_md5}` |
| **Hash SHA-256** | `{data.hash_sha256}` |
| **Data da Análise** | {data.timestamp} |
| **Modo de Análise** | {data.analysis_mode} |
| **Modelo LLM** | {data.model} |
| **Engine OCR** | {data.ocr_engine} |
| **Tempo Total** | {data.processing_time:.2f}s |

{self._chain_of_custody_telemetry(data)}
