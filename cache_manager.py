...
# Diretório de cache. Ancorado no módulo, e não no CWD: a CLI pode ser executada de
# qualquer diretório e um caminho relativo criaria um cache novo e vazio em silêncio.
CACHE_DIR = Path(os.getenv("VISION_CACHE_DIR") or (Path(__file__).resolve().parent / ".vision_cache"))
CACHE_INDEX_FILE = CACHE_DIR / "cache_index.json"
STATS_FILE = CACHE_DIR / "stats_history.json"

# Teto do histórico de métricas. Sem limite, o arquivo cresce sem fim e cada análise
# reescreve o JSON inteiro.
...
class CacheManager:
    ...
    def __init__(self, cache_dir: Path = CACHE_DIR):
        ...
        self.stats_file = self.cache_dir / "stats_history.json"
        # O lote roda com até 8 workers em ThreadPoolExecutor e todos escrevem no mesmo
        # índice; sem o lock, duas gravações simultâneas corrompem o JSON.
    ...
    def _write_json_atomic(target: Path, payload) -> None:
        """Grava JSON de forma atômica: escreve num temporário e troca por os.replace.

        Evita deixar o arquivo truncado se o processo morrer no meio da escrita.
        """
    ...
    def _save_index(self):
        """Salva o índice de cache no disco. Chamar sempre com o lock adquirido."""
        try:
            self._write_json_atomic(self.cache_index_file, self.index)
        except Exception as e:
    ...
    def _save_stats(self):
        """Salva histórico de estatísticas. Chamar sempre com o lock adquirido."""
        try:
            self._write_json_atomic(self.stats_file, self.stats_history)
        except Exception as e:
    ...
    def record_analysis(self, model: str, processing_time: float, cache_hit: bool = False):
        ...
        with self._lock:
            ...
            if len(self.stats_history) > MAX_STATS_ENTRIES:
                self.stats_history = self.stats_history[-MAX_STATS_ENTRIES:]
            self._save_stats()

    def get_stats_history(self) -> list:
        """Retorna uma cópia do histórico de métricas."""
        with self._lock:
